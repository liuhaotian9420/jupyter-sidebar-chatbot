 ...
 */
export function buildLayout(callbacks: LayoutCallbacks = {}): LayoutElements {
  // --- Main Content Wrapper ---
  const mainElement = createDiv({
    classes: 'jp-llm-ext-content-wrapper'
  });

  // --- Title Bar ---
  const titleContainer = createDiv({ classes: 'jp-llm-ext-title-container' });
  ...
  const messageContainer = createDiv({ classes: 'jp-llm-ext-message-container' });
  const historyContainer = createDiv({
  ...
  const controlsRow = createDiv({ classes: 'jp-llm-ext-bottom-bar-row jp-llm-ext-controls-row' });
  const controlsContainer = createDiv({ classes: 'jp-llm-ext-controls-container' });

  // Markdown Toggle
  const toggleContainer = createDiv({ classes: 'jp-llm-ext-toggle-container' });
  const markdownToggleButton = createInputElement({
    id: 'markdown-toggle',
    attributes: { type: 'checkbox' }
  });
  const toggleLabel = createLabelElement({
    text: 'Markdown mode',
  ...
  const actionButtonsContainer = createDiv({ classes: 'jp-llm-ext-action-buttons-container' });

  const atButton = createButton({ text: '@', attributes: { title: 'Browse cells, code, files, and more' }, classes: 'jp-Button jp-llm-ext-action-button' });
  if (callbacks.onAtButtonClick) {
    atButton.addEventListener('click', callbacks.onAtButtonClick);
  }

  const expandButton = createButton({ text: '⤢', attributes: { title: 'Expand input' }, classes: 'jp-Button jp-llm-ext-action-button' });
  if (callbacks.onExpandToggleClick) {
    expandButton.addEventListener('click', () => callbacks.onExpandToggleClick!(expandButton));
  }

  const settingsButton = createButton({ text: '⚙️', attributes: { title: 'Settings' }, classes: 'jp-Button jp-llm-ext-action-button' });
  ...
  const buttonsRow = createDiv({ classes: 'jp-llm-ext-bottom-bar-row jp-llm-ext-buttons-row' });
