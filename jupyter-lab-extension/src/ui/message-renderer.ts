 ...
 */
export function renderReferenceWidgetInline(
    type: 'code' | 'cell' | 'file' | 'dir',
    data: CodeRefData | string, // string for file/dir paths
    placeholder: string, // ADDED: The original placeholder text (e.g., @code[ref-1])
    refId?: string // Keep refId separate for map lookups if needed
): HTMLElement {
    const widget = document.createElement('span');
    ...
    switch (type) {
        ...
        case 'cell': { // Combine logic for code and cell previews
            const refData = data as CodeRefData;
            ...
            if (refId) widget.dataset.refId = refId;
