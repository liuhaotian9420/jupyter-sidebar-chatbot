...
def TestArrays(vbtest, bUseGenerated):
    ...
    if bUseGenerated:
        ...
        resultData, byRefParam = vbtest.PassSAFEARRAYVariant(testData)
        assert testData == list(byRefParam), f"Expected '{testData}', got '{list(byRefParam)}'"
        assert testData == list(resultData), f"Expected '{testData}', got '{list(resultData)}'"
...
def TestStructs(vbtest):
    ...
    assert sub.int_val == 22, (
        "The sub-struct value didn't persist!",
        str(sub.int_val),
    )
    ...
    expected = (
        f"com_struct(int_val={s.int_val!r}, str_val={s.str_val!r}, ob_val={s.ob_val!r}, sub_val={s.sub_val!r})"
    )
    repr_s = repr(s)
    if repr_s != expected:
